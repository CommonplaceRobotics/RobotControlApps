...
from irc_app.DataTypes.Matrix44 import Matrix44FromGrpc
from irc_app.rpc.robotcontrolapp_pb2 import AppFunction, AppUIElement, ButtonState, CheckboxState
...
class MinimalApp(AppClient):
    ...
    def __init__(self, appName: str, target: str):
        """
        Initializes the app. Pass the app name (as defined in rcapp.xml) and
        socket to connect to (default: "localhost:5000")
        """
    ...
    def ExamplePrintAppFunctionParameters(self, function):
        """
        This example prints all app functions contained in an AppFunction
        request (e.g. coming from the App program command)
        """
        print(
        ...
        for parameter in function.parameters:
            if parameter.HasField("bool_value"):
                ...
                matrix = Matrix44FromGrpc(parameter.cartesian_value)
                print(
    ...
    def ExampleExponentiation(self, function):
        """
        This example exponentiates the value of a number variable by a given exponent
        and assigns the result to a different number variable
        """
    ...
    def ExampleUIElementClicked(self, updates):
        ...
        for update in updates:
            # The following sample will increase/decrease a value if the
            # plus/minus button is pressed and send it back to the UI.
