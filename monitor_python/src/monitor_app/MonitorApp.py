...
from irc_app.DataTypes.RobotState import RobotState
...
class MonitorApp(AppClient):
    ...
    def OnRobotStateUpdated(self, state: RobotState):
        """
        Is called when the robot state is updated (usually each 10 or 20ms). Override this
        method, start the stream by calling StartRobotStateStream().
        """
        self.UpdateRobotState(state)
